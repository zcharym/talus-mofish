...
CREATE UNIQUE INDEX IF NOT EXISTS idx_cards_anki_card_id ON cards(anki_card_id) WHERE anki_card_id IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_cards_due ON cards(deck_id, due_at) WHERE suspended = 0;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_vocabulary_anki_note_guid ON vocabulary(anki_note_guid) WHERE anki_note_guid IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS articles (
    ...
    source         TEXT NOT NULL DEFAULT 'manual'
                       CHECK (source IN ('manual', 'clipboard', 'web', 'youtube', 'import:anki')),
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_articles_anki_note_guid ON articles(anki_note_guid) WHERE anki_note_guid IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS anki_import_decks (
    ...
    target_type    TEXT NOT NULL
                     CHECK (target_type IN ('vocabulary', 'reading', 'skip')),
